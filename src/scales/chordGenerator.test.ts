...
function noteToMidi(abcNote: string): number {
  const acc = abcNote.startsWith('^^') ? 2 : abcNote.startsWith('__') ? -2 : abcNote.startsWith('^') ? 1 : abcNote.startsWith('_') ? -1 : 0;
...
describe('generateChordAbc — traditional range', () => {
  it('C major triad on SOPRANO: first note is C (uppercase, octave 4)', () => {
    ...
    const result = generateChordAbc({ chord, range: 'traditional', direction: 'ascending', instrumentType: 'SOPRANO' });
    expect(result.trim().split(/\s+/)[0]).toBe('C');
  });

  it('C major triad: SOPRANO and TENOR produce identical notation', () => {
    const chord = { name: 'C', notes: buildChord('C', [0, 4, 7]) };
    const soprano = generateChordAbc({ chord, range: 'traditional', direction: 'ascending', instrumentType: 'SOPRANO' });
    const tenor = generateChordAbc({ chord, range: 'traditional', direction: 'ascending', instrumentType: 'TENOR' });
    expect(soprano).toBe(tenor);
  });

  it('C major triad on BASS: first note is C,, (octave 2)', () => {
    const chord = { name: 'C', notes: buildChord('C', [0, 4, 7]) };
    const result = generateChordAbc({ chord, range: 'traditional', direction: 'ascending', instrumentType: 'BASS' });
    expect(result.trim().split(/\s+/)[0]).toBe('C,,');
  });
  ...
  it('descending: notes are reverse of ascending', () => {
    const chord = { name: 'C', notes: buildChord('C', [0, 4, 7]) };
    const asc = generateChordAbc({ chord, range: 'traditional', direction: 'ascending', instrumentType: 'SOPRANO' });
    const desc = generateChordAbc({ chord, range: 'traditional', direction: 'descending', instrumentType: 'SOPRANO' });
  ...
  it('random: same notes as ascending, shuffled', () => {
    const chord = { name: 'C', notes: buildChord('C', [0, 4, 7]) };
    const asc = generateChordAbc({ chord, range: 'traditional', direction: 'ascending', instrumentType: 'SOPRANO' });
    const rand = generateChordAbc({ chord, range: 'traditional', direction: 'random', instrumentType: 'SOPRANO' });
    const ascNotes = asc.trim().split(/\s+/).filter(Boolean);
...
describe('generateChordAbc — all range', () => {
  it('G major: SOPRANO and TENOR produce identical notation', () => {
    ...
    const soprano = generateChordAbc({ chord, range: 'all', direction: 'ascending', instrumentType: 'SOPRANO' });
    const tenor = generateChordAbc({ chord, range: 'all', direction: 'ascending', instrumentType: 'TENOR' });
    expect(soprano).toBe(tenor);
  });

  it('G major: SOPRANINO and ALTO produce identical notation', () => {
    const chord = { name: 'G', notes: buildChord('G', [0, 4, 7]) };
    const sopranino = generateChordAbc({ chord, range: 'all', direction: 'ascending', instrumentType: 'SOPRANINO' });
    const alto = generateChordAbc({ chord, range: 'all', direction: 'ascending', instrumentType: 'ALTO' });
    expect(sopranino).toBe(alto);
  });

  it('G major on TENOR: all notes within written range (MIDI 60–85)', () => {
    const chord = { name: 'G', notes: buildChord('G', [0, 4, 7]) };
    const result = generateChordAbc({ chord, range: 'all', direction: 'ascending', instrumentType: 'TENOR' });
    const tokens = result.trim().split(/\s+/).filter(Boolean);
    for (const token of tokens) {
      const midi = noteToMidi(token);
      expect(midi).toBeGreaterThanOrEqual(60);
      expect(midi).toBeLessThanOrEqual(85);
    }
  });
